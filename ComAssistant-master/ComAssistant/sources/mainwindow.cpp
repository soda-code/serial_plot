...
void MainWindow::debugTimerSlot()
{
    ...
    num5 = static_cast<float>(qSin(count / 0.3843) +QRandomGenerator::global()->generateDouble() * qSin(count / 0.6157));
    //num5 = static_cast<float>(qSin(count/0.3843)+qrand()/static_cast<double>(RAND_MAX)*1*qSin(count/0.6157));
    num6 = static_cast<float>(qCos(count / 0.3843) +QRandomGenerator::global()->generateDouble() * qCos(count / 0.6157));
...
void MainWindow::readSerialPort()
{
    ...
    if(ui->timeStampCheckBox->isChecked() && timeStampTimer.isActive()==false){
       ...
       // hexBrowserBuff.append(timeString + toHexDisplay(tmpReadBuff).toLatin1());//换行符在前面判断没有数据时自动追加一次
        hexBrowserBuff.append(timeString.toUtf8());
        ...
        QString combined = timeString + QString::fromLocal8Bit(tmpReadBuff);
        BrowserBuff.append(combined.toLocal8Bit());
...
void MainWindow::on_sendButton_clicked()
{
    ...
    if(ui->timeStampCheckBox->isChecked()){
        ...
        timeString = QDateTime::currentDateTime().toString("hh:mm:ss.zzz");
        timeString = "\n["+timeString+"]Tx-> ";
        hexBrowserBuff.append(timeString.toUtf8() + toHexDisplay(sendArr).toUtf8());
        // hexBrowserBuff.append(timeString + toHexDisplay(sendArr));
        //BrowserBuff.append(timeString + QString::fromLocal8Bit(sendArr));
        BrowserBuff.append(timeString.toUtf8() + QString::fromLocal8Bit(sendArr).toUtf8());
        //打印数据
        printToTextBrowser();
    }
...
void MainWindow::on_actionUTF8_triggered(bool checked)
{
    ...
    if(checked){
        //设置中文编码
        ui->actionGBK->setChecked(false);  // 禁用 GBK 编码操作
#if 0
        QString  *codec = QTextCodec::codecForName("UTF-8");
...
void MainWindow::on_actionSaveOriginData_triggered()
{
    ...
    if(file.open(QFile::WriteOnly|QFile::Truncate)){
        ...
        BrowserBuff.append(data);
        QByteArray data_1 = str.toLocal8Bit();
        hexBrowserBuff.append(toHexDisplay(data_1).toUtf8());
        printToTextBrowser();
    }
...
{   
    ...
    if(file.open(QFile::ReadOnly)){
        ...
        BrowserBuff.clear();
        QString text = ui->textBrowser->document()->toPlainText();  // 获取文本框中的文本
        BrowserBuff.append(text.toUtf8());  // 将 QString 转换为 QByteArray 并追加到 BrowserBuff
...
void MainWindow::on_actionSaveShowedData_triggered()
{
    ...
    if(file.open(QFile::WriteOnly|QFile::Text|QFile::Truncate)){
        ...
        BrowserBuff.append(str.toUtf8());
        hexBrowserBuff.append(toHexDisplay(str.toLocal8Bit()).toUtf8());
        printToTextBrowser();
    }
}
